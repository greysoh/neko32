...
fn get_instruction_length(instruction: u32) -> u8 {
    ...
    if instruction == Opcodes::NOP || instruction == Opcodes::SPO {
        return 0;
    }

    if instruction == Opcodes::EQL || instruction == Opcodes::GHT {
        return 3;
    }
    if instruction >= Opcodes::MEW && instruction <= Opcodes::MCP {
        return 2;
    }

    if instruction == Opcodes::SPU || instruction == Opcodes::SPE {
        return 1;
    }
    if instruction >= Opcodes::LSB && instruction <= Opcodes::MOD {
        return 3;
    }

    if instruction == Opcodes::REW {
        return 5;
    }
    if instruction == Opcodes::FUN {
        return 4;
    }
    if instruction == Opcodes::INV {
        return 2;
    }
    if instruction == Opcodes::RET {
        return 1;
    }
...
pub fn decode(fetched_instruction: Vec<u8>) -> Instruction {
    ...
    if u32::from(fetched_instruction[0]) == Opcodes::REW {
        ...
        argv.push(u32_to_int(
            &fetched_instruction[1..(instruction_length + 1) as usize],
        ));
...
pub fn execute(
    ...
    match instruction.opcode {
        ...
        Opcodes::EQL => {
            registers[instruction.argv[2] as usize] = (registers[instruction.argv[0] as usize]
                == registers[instruction.argv[1] as usize])
                as u32;
        }
        ...
        Opcodes::MEW => {
            ...
            memory.set(
                instruction.argv[0],
                registers[instruction.argv[1] as usize] as u8,
            );
        }
        ...
        Opcodes::MCP => {
            ...
            registers[instruction.argv[1] as usize] =
                memory.get(registers[instruction.argv[0] as usize]) as u32;
        }

        Opcodes::SPU => {
            if registers[1] > 9999 {
                panic!("Stack push: Limit over threshold in vCPU stack")
            }
            if registers[1] < 8191 {
                panic!("Stack push: Limit under threshold in vCPU stack")
            }
        ...
        Opcodes::LSB => {
            registers[instruction.argv[2] as usize] = overflow_resolver(
                (registers[instruction.argv[0] as usize] as u64)
                    << registers[instruction.argv[1] as usize] as u64,
            );
        }
        ...
        Opcodes::NOT => {
            registers[instruction.argv[1] as usize] =
                overflow_resolver((!registers[instruction.argv[0] as usize]) as u64);
        }
